...
export function convertToSvg(db, transformStack = [], visibleLayers = null) {
  ...
  // If no visibleLayers specified, show all layers
  ...
  const renderedHandles = new Set();
  const blockIdCounter = { value: 0 };
  const skippedByLayer = new Map(); // Track entities skipped by layer
  ...
  // Updated layer visibility check - now uses the visibleLayers array
  const isLayerVisible = (layerName) => {
    if (shouldShowAllLayers) {
      return true; // Show all layers if no filter specified
    }
    
    if (!layerName) {
      return true; // Show entities without layer specification
    }
    
    const isVisible = visibleLayers.includes(layerName);
    
    if (!isVisible) {
      // Track skipped layers for debugging
  ...
  // Updated entity rendering check
  const shouldRenderEntity = (entity, layers = {}) => {
    // First check layer visibility using our visibleLayers array
    if (entity.layer && !isLayerVisible(entity.layer)) {
      console.log(`Skipping entity on hidden layer: ${entity.layer} (type: ${entity.type})`);
      return false;
    }

    // Then check other visibility flags
    const layerInfo = layers[entity.layer];
    if (layerInfo) {
      // Check layer frozen/off flags from DWG layer table
      if (layerInfo.frozen === true || (layerInfo.flags && layerInfo.flags & 4)) return false; // Frozen
      if (layerInfo.off === true || (layerInfo.flags && layerInfo.flags & 2)) return false; // Off
      if (layerInfo.flags && layerInfo.flags & 1) return false; // Frozen in new viewports
    }
  ...
  const analyzeAllEntities = (entities) => {
    ...
    console.log('Entity type counts:', typeCount);
    console.log('Entity layer counts:', layerCount);
    console.log('Potential circular entities:', circularEntities);
  ...
  const debugInsertElement = (e, source, currentTransforms) => {
    ...
    console.log(`INSERT scale:`, { x: e.xScale, y: e.yScale });
    console.log(`INSERT rotation:`, e.rotation);
    console.log(`INSERT layer:`, e.layer);
  ...
  const entityHandlers = {
    ...
    INSERT: (e, color, stroke, transforms) => {
      ...
      // Process all entities in the block with the new transform
      const blockContent = [];
      for (const entity of blockEntities) {
        if (!entity?.type) continue;

        // Check if this entity should be rendered (layer visibility check)
  ...
  const generateElement = (e, source, currentTransforms) => {
    ...
    // Create layers lookup for shouldRenderEntity check
    const layerInfo = tables.LAYER?.entries?.reduce((acc, layer) => {
      acc[layer.name] = layer;
      return acc;
    }, {}) || {};
...
${regularElements.join('\n')}
  ...
  const generateInsertUseElement = (e, source, currentTransforms) => {
    ...
    // Check if INSERT entity itself should be rendered (layer check)
    const layerInfo = tables.LAYER?.entries?.reduce((acc, layer) => {
      acc[layer.name] = layer;
      return acc;
    }, {}) || {};
    ...
    console.log(`INSERT ${blockName}: pos(${insertPoint.x}, ${insertPoint.y}), scale(${xScale}, ${yScale}), rot(${rotation}), layer=${e.layer}`);
